        ...
        .then(res => {
            if(res.errcode===0){
                ...
                if(Number(that.innerHTML)>0){
                    that.innerHTML=Number(that.innerHTML) - 1
                }
            }
        })
        .catch(error => {
...
function checkPhone() {
	...
	if(cellphone.length <= 0)
	{
		document.querySelector("#log-box .cellerr").innerHTML = '请填写您的手机号码';
		return false;
	}else if(!phonepreg.test(cellphone)){
		document.querySelector("#log-box .cellerr").innerHTML = '手机号码格式不正确';
		return false;
	}else{
		document.querySelector("#log-box .cellerr").innerHTML = '';
	}
