...
function setLike(id,that,token) {
    // 使用dataset存储like状态
    if(that.dataset.like === "like"){
        // 不喜欢
        var xhr = new XMLHttpRequest();
        xhr.open('post', '/note/unlike', true);
        ...
        xhr.onreadystatechange = function() {
            if(xhr.readyState === 4 && xhr.status === 200) {
                ...
                if(res.errcode === 0){
                    ...
                    that.classList.remove("like");
                    that.classList.add("not-like");
                    that.dataset.like = "not-like";
                    if(Number(that.textContent) > 0){
                        that.textContent = Number(that.textContent) - 1;
                    }
                    that.setAttribute("onclick","setLike("+id+",this,'" + token + "')");
                    tip({text: '已取消点赞', time: 1500});
                } else {
                    tip({text: res.errmsg || '操作失败，请重试', time: 1500});
                }
            }
        };
        xhr.send('id='+encodeURIComponent(id)+'&_token='+encodeURIComponent(token));
    }else{
        // 喜欢
        var xhr = new XMLHttpRequest();
        xhr.open('post', '/note/like', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if(xhr.readyState === 4 && xhr.status === 200) {
                var res = JSON.parse(xhr.responseText);
                if(res.errcode === 0){
                    that.setAttribute("onclick","");
                    that.classList.remove("not-like");
                    that.classList.add("like");
                    that.dataset.like = "like";
                    that.textContent = Number(that.textContent) + 1;
                    that.setAttribute("onclick","setLike("+id+",this,'" + token + "')");
                    tip({text: '点赞成功', time: 1500});
                } else {
                    tip({text: res.errmsg || '操作失败，请重试', time: 1500});
                }
            }
...
//上传食材图片
var addscpicInputs = document.querySelectorAll(".addscpic_input");
for(var i=0; i<addscpicInputs.length; i++){
    addscpicInputs[i].addEventListener("change", function(e){
...
function checkPhone() {
    ...
    var phonepreg = /^1[3456789]\d{9}$/;
    var cellerrElements = document.querySelectorAll("#log-box .cellerr");
